############################################################################
# Functions used by several scripts of this folder to process all the variants
# of the Straightness on a collection of graphs.
#
# Vincent Labatut 09/2016
#
# setwd("~/eclipse/workspaces/Networks/SpatialMeasures")
# setwd("d:/eclipse/workspaces/Networks/SpatialMeasures")
# source("src/figures/common.R")
############################################################################
...
############################################################################
# Process all the average straightness variants for all the specified
# types of graphs.
# 
# graph.types: list of graph names.
# folder: folder in which to find the graphs/record the results.
############################################################################
com.process.straightness <- function(graph.types, folder)
{	for(gtype in graph.types)
	{	########################################
		# Load the (previously created) graph
		########################################
		tlog("Load the ",gtype," graph")
		out.folder <- file.path(folder,gtype)
...
############################################################################
# Check if the Straightness values are within the theoretical ranges (used 
# when debugging).
# 
# graph.types: list of graph names.
# folder: folder in which to find the graphs/record the results.
############################################################################
com.check.straightness <- function(graph.types, folder)
{	for(gtype in graph.types)
