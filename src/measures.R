############################################################################
# Processing of various topological measures.
#
...
############################################################################
# Processes the total (spatial) lenght of all edges in the graph.
#
# Note : if the graph already has a "dist" edge attribute, it is used.
#
...
#  2) Get the spatial length of the shortest path between each pair of node.
#  3) Divide the first by the second.
# If the node is not connected, then its straightness is zero.
#
# g: the graph to process.
# unilateral: NA to process the bilateral version of the measure, or a node id
#			  to process the unilateral version, considering this node as the 
#			  network center.
# 
# returns: for the bilateral straightness, a matrix whose cells correspond 
#		   to straightness values for each pair of nodes in the graph. For the
#		   unilateral straightness, a vector of the straightness betwen the
#		   network center and each of the n nodes of the graph (including the
#		   center node itself).
############################################################################
node.straightness <- function(g, unilateral=NA)
{	# bilateral version
	if(is.na(unilateral))
	...
	{	# process spatial distances
		pos <- cbind(V(g)$x,V(g)$y)
		pos[,1] <- pos[,1] - V(g)$x[unilateral]
		pos[,2] <- pos[,2] - V(g)$y[unilateral]
		pos <- pos * pos
		m <- sqrt(pos[,1] + pos[,2])
		
		# process geodesic distances
		g <- distances.as.weights(g)
		sp <- shortest.paths(graph=g, v=unilateral, weights=E(g)$dist)
...
############################################################################
# Processes the average straightness over all pairs of nodes in the specified
# graph. Also returns the standard deviation. Straigthness between one node
# and itself is taken into account.
# 
# g: graph to consider.
# unilateral: NA to process the bilateral version of the measure, or a node id
#			  to process the unilateral version, considering this node as the 
#			  network center.
# 
# returns: a vector of two values: average straightness and standard deviation,
#		   both processed over all pairs of nodes in the specified graph. 
############################################################################
mean.node.straightness <- function(g, unilateral=NA)
{	# all straightness values for the graph
	strn <- node.straightness(g,unilateral)
	if(is.na(unilateral))
#		strn <- strn[upper.tri(strn,diag=FALSE)]	# bilateral: we keep half the (symmetrical) matrix (no diagonal)
		strn <- strn[upper.tri(strn,diag=TRUE)]
	else
#		strn <- strn[-1]							# unilateral: we remove the straightness of the center
		strn <- strn
	
	# old version
#	strn <- node.straightness(g,FALSE)
#	if(unilateral)
#		strn <- strn[1,]				# unilateral: we only keep pairs containing the center
#	else
#		strn <- strn[upper.tri(strn)]	# bilateral: we keep all pairs, i.e. half the (symmetrical) matrix
	
	# average straightness
	res1 <- mean(strn)
	
	# standard deviation of the straightness
	res2 <- sd(strn)
	
	return(c(res1,res2))
}
mean.node.straightness2 <- function(g, unilateral=NA)
{	g <- distances.as.weights(g)
	...
	{	xc <- V(g)$x[nc]						# get the spatial position of node C
		yc <- V(g)$y[nc]
#		cat("Node ",nc,"/",vcount(g),"\n",sep="")
		...
		{	na <- el[e,1]						# get node A
			...
			xb <- V(g)$x[nb]
			yb <- V(g)$y[nb]
			dgca <- sp[nc,na] 					# geodesic distance between C and A
			dgcb <- sp[nc,nb]					# geodesic distance between C and B
			#TODO should check if these geodesic distances are defined (i.e.: what about disconnected graphs?)
			ddab <- E(g)$dist[e]				# Euclidean distance between A and B
			ddca <- sqrt((xa-xc)^2+(ya-yc)^2)	# Euclidean distance between C and A
			ddcb <- sqrt((xb-xc)^2+(yb-yc)^2)	# Euclidean distance between C and B
#			cat("Node ",nc," vs. Link (",na,",",nb,"):\n",sep="")
#			cat("dgca: ",dgca," dgcb: ",dgcb,"\n",sep="")
#			cat("ddca: ",ddca," ddcb: ",ddcb," ddab: ",ddab,"\n",sep="")
#if(na==2 && nb==14)
#	print("++++++++++++++++++++++++++++++++++++++++++")
			
			# primitive undefined for certain specific cases 
			# fortunately, straightness processing is trivial for them
#print(check.alignment(xa,ya,xb,yb,xc,yc))
#print(abs(ddca-dgca)<1e-10)
#print(abs(ddcb-dgcb)<1e-10)
			if(na==nc || nb==nc || 
				(check.alignment(xa,ya,xb,yb,xc,yc) && (abs(ddca-dgca)<1e-10 || abs(ddcb-dgcb)<1e-10)))
			{	# the straightness is by definition one over the whole link
				pus <- ddab
			}
			# general case: use the primitive
			else
...
#		cat("..",total.straightness," vs. ",total.length,"\n",sep="")
	}
	
	return(c(total.straightness/total.length,0))
}
...
#
# returns: avector containing the mean distance and the associated standard deviation.
############################################################################
